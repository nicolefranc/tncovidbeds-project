...
from constants import BODY_STR, DISTRICT_SHORTS
...
def bot():
    ...
    incoming = request.values.get(BODY_STR, '').lower().strip()
    print(incoming)
    ...
    if '-' in incoming:
        try:
            incoming = int(incoming.split('-')[1].strip())
        except:
        ...
        if incoming.upper() in DISTRICT_SHORTS:
            out, out2, out3, out4, out5, out6, out7, out8 = listHospitals(
                incoming)
            message.body(out)
            if out2 != '':
